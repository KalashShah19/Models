...
# Assume the news data CSV has columns: 'Date', 'Sentiment'
news_data = pd.read_csv('news_data.csv', parse_dates=['Date'])

# Step 3: Preprocess and merge news data
# Ensure the news data has the same frequency as stock data
news_data.set_index('Date', inplace=True)
news_data = news_data.resample('D').mean()  # Resample to daily frequency, adjust if necessary

# Merge stock data with news data
stock_data = stock_data.join(news_data, how='left')

# Fill NaN values (optional, e.g., forward-fill)
stock_data.fillna(method='ffill', inplace=True)

# Step 4: Add Technical Indicators (SMA, RSI)
...
stock_data.to_csv('D:/! Kalash/AI/Models/Stocks/Data/vedanta_news.csv')
